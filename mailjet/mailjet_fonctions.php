...
function mailjet_get_iframe_lang($langCode = null){
...
if (!defined("_ECRIRE_INC_VERSION")) return;
	...
	*/
function mailjet_convertir_styles_inline($body){
	...
	foreach ($styles[1] as $i => $type) {
		if ($type=="" && $styles[3][$i]=="") {
			...
			if (!preg_match("/ /", $styletag) && !preg_match("/#/", $styletag)) {
				$pattern = "!<(".$styletag.")([^>]*(?= /)|[^>]*)( /)?>!mi";
				$body = preg_replace_callback ($pattern, 'mailjet_addstyle' , $body);
 ...
 * @return string
 */
function mailjet_addstyle($matches) {
 ...
 * @return string
 */
function mailjet_nl2br_si_pas_autobr($texte){
