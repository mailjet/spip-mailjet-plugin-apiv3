...
include_spip('mailjet_fonctions');



class Mailjet extends PHPMailer {

	function Mailjet($email, $objet, $message_html, $message_texte) {
		...
		if ($GLOBALS['meta']['mailjet_adresse_envoi'] == 'oui'
		  AND $GLOBALS['meta']['mailjet_adresse_envoi_email'])
			$this->From = $GLOBALS['meta']['mailjet_adresse_envoi_email'];
		else
		...
		if (strpos($this->From,",")===false){
			if ($GLOBALS['meta']['mailjet_adresse_envoi'] == 'oui'
			  AND $GLOBALS['meta']['mailjet_adresse_envoi_nom'])
				$this->FromName = $GLOBALS['meta']['mailjet_adresse_envoi_nom'];
			else
		...
		if (is_array($email)) {
			foreach ($email as $cle => $adresseMail) {
				if (!$this->AddAddress($adresseMail))
					spip_log("Erreur AddAddress $adresseMail : ".print_r($this->ErrorInfo,true),'mailjet');
			}
		}
		else
			if (!$this->AddAddress($email))
				spip_log("Erreur AddAddress $email : ".print_r($this->ErrorInfo,true),'mailjet');

		if (!empty($GLOBALS['meta']['mailjet_smtp_sender'])) {
			$this->Sender = $GLOBALS['meta']['mailjet_smtp_sender'];
			$this->AddCustomHeader("Errors-To: ".$this->Sender);
		}

		if (!empty($GLOBALS['meta']['mailjet_cc'])) {
			$this->AddCC( $GLOBALS['meta']['mailjet_cc'] );
		}
		if (!empty($GLOBALS['meta']['mailjet_bcc'])) {
			$this->AddBCC( $GLOBALS['meta']['mailjet_bcc'] );
		}
		
        $this->Mailer	= 'smtp';
        $this->Host 	= MAILJET_HOST;
        $this->Port 	= $GLOBALS['meta']['mailjet_smtp_port'];
        $this->SMTPAuth = true;
        $this->Username = $GLOBALS['meta']['mailjet_smtp_username'];
        $this->Password = $GLOBALS['meta']['mailjet_smtp_password'];
        if (intval(phpversion()) == 5) {
        if ($GLOBALS['meta']['mailjet_smtp_secure'] == 'ssl')
            $this->SMTPSecure = 'ssl';
        if ($GLOBALS['meta']['mailjet_smtp_secure'] == 'tls')
            $this->SMTPSecure = 'tls';
        }
		...
		if (!empty($message_html)) {
			...
			$this->IsHTML(true);
			if ($GLOBALS['meta']['mailjet_filtre_images'])
		...
		if ($GLOBALS['meta']['mailjet_filtre_iso_8859'])
	...
	function ConvertirStylesEnligne() {
		$this->Body = mailjet_convertir_styles_inline($this->Body);
	}
	...
	public function Send() {
		...
		ob_end_clean();
		if( !empty($error) ) {
			spip_log("Erreur Mailjet->Send : $error",'mailjet.err');
		}
		return $retour;
	}
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
		...
		ob_end_clean();
		if( !empty($error) ) {
			spip_log("Erreur Mailjet->AddAttachment : $error",'mailjet.err');
		}
		return $retour;
	}
	public function AddReplyTo($address, $name = '') {
		...
		ob_end_clean();
		if( !empty($error) ) {
			spip_log("Erreur Mailjet->AddReplyTo : $error",'mailjet.err');
		}
		return $retour;
	}
	public function AddBCC($address, $name = '') {
		...
		ob_end_clean();
		if( !empty($error) ) {
			spip_log("Erreur Mailjet->AddBCC : $error",'mailjet.err');
		}
		return $retour;
	}
	public function AddCC($address, $name = '') {
		...
		ob_end_clean();
		if( !empty($error) ) {
			spip_log("Erreur Mailjet->AddCC : $error",'mailjet.err');
		}
		return $retour;
	}
}
