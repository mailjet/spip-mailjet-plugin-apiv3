...
function inc_envoyer_mail($destinataire, $sujet, $corps, $from = "", $headers = "") {
	...
	// On crée l'objet Mailjet (PHPMailer) pour le manipuler ensuite
	$mailjet = new Mailjet($destinataire, $sujet, $message_html, $message_texte);
	
	// On ajoute le courriel de l'envoyeur s'il est fournit par la fonction
	if (empty($from) AND empty($mailjet->From)) {
	...
	if (count($pieces_jointes)) {
		foreach ($pieces_jointes as $piece) {
			$mailjet->AddAttachment(
				...
				isset($piece['mime']) ? $piece['mime']:Mailjet::_mime_types(pathinfo($piece['chemin'], PATHINFO_EXTENSION))
			);
		}
	}
	...
	if (!empty($adresse_erreur))
		$mailjet->Sender = $adresse_erreur;

	// si entetes personalises : les ajouter
	// bug : semble ecraser les autres headers. A debug si on veut le rendre fonctionnel
	//if (!empty($headers)) {
	//	foreach($headers as $h)
	//		$mailjet->AddCustomHeader($h);
	//}
	
	// On passe dans un pipeline pour modifier tout le mailjet avant l'envoi
	$mailjet = pipeline('mailjet_pre_envoi', $mailjet);
	
	// On génère les headers
	$head = $mailjet->CreateHeader();
	...
	if (!$retour)
		spip_log("Erreur Envoi mail via Mailjet : ".print_r($mailjet->ErrorInfo,true),'mailjet');
